...
# Naver
def naver_redirect(request: HttpRequest) -> HttpResponse:
    """
    Once receiving the authorization code from Naver, this function will:
    - extract request parameters (state, a.k.a. frontend redirect uri, (authorization) code, error)
    - call POST `https://nid.naver.com/oauth2.0/token` api to retrieve access & refresh token
    
    Once receiving the token from Naver, this function will:
    - extract access & refresh tokens and all its relevant info
    - call GET `https://openapi.naver.com/v1/nid/me` to fetch the current user's info
    - based on this info, if the user is already existing, then redirect to the frontend_redirect_uri?status="existing"
    - otherwise, it will redirect to status="new"
    - in case of error, it will throw an exception
    ...
    state = request.GET.get("state") # for the frontend redirect uri + CSRF check (need to be same as the one we sent from FO to kakao api)
    frontend_redirect_uri = unquote(state)
    authorization_code = request.GET.get("code") # from Naver GET api response
    auth_error_code = request.GET.get("error") # error code from Naver GET api
    auth_error_description = request.GET.get("error_description", "Failed to retrieve authorization code")
    ...
    token_response_json = token_response.json()
    access_token = token_response_json.get("access_token")
    error_code = token_response_json.get("error")
    error_description = token_response_json.get("error_description")

    # Step 3. Handle the GET api response
    if error_code or not token_response.ok or not access_token:
        return JsonResponse({"error": f"Failed to retrieve access token, {error_description}"}, status=token_response.status_code)

    # Step 3.1: Retrieve current user info
    naver_user_info_url = "https://openapi.naver.com/v1/nid/me"
    naver_user_info_headers = {
        "Authorization": f"Bearer {access_token}",
    ...
    user_info_response = requests.get(naver_user_info_url, headers=naver_user_info_headers)
    user_info_response_json = user_info_response.json()

    user_naver_info = user_info_response_json.get("response")
    user_naver_id = user_naver_info.get("id")

    if not user_info_response.ok or not user_naver_id:
        return JsonResponse({"error": "Failed to retrieve user info"}, status=user_info_response.status_code)

    # Step 3.1.1: Check the gender
    gender = user_naver_info.get("gender")
    if not gender or gender != "F":
        return JsonResponse({"error": "Only female users are allowed to sign in"}, status=status.HTTP_401_UNAUTHORIZED)

    # Step 3.2: Check if the current user already exists
    # (Django model lookup will raise DoesNotExist or MultipleObjectsReturned)
    try:
        auth_item = Authentication.objects.get(provider_user_id=user_naver_id, auth_type=AuthType.NAVER)
        # If exists, then get the actual user ID
        if auth_item:
            print("User with this Naver account found")
            try:
                user_auth_item = UserAuthentication.objects.get(auth_id=auth_item.auth_id)
                user_id = user_auth_item.user_id
                ...
                try:
                    user_item = UserDetail.objects.get(
                        name=user_naver_info.get("name"),
                        username=user_naver_info.get("nickname")
                    )
                except ObjectDoesNotExist:
                    ...
                    user_item = UserDetail.objects.create(
                        name=user_naver_info.get("name"),
                        username=user_naver_info.get("nickname"),
                        age_range=user_naver_info.get("age"),
        ...
        print("User with this Naver account not found")
        # Otherwise, create a new user object
        # New Authentication object
        refresh_token = token_response_json.get("refresh_token")
        token_expiration = int(token_response_json.get("expires_in",0))

        new_auth = Authentication.objects.create(
            auth_type = AuthType.NAVER,
            provider_user_id=user_naver_id,
            provider_access_token = access_token,
            provider_refresh_token = refresh_token,
        )
        new_auth.set_token_expiration(token_expiration, 'access')
        new_auth.save()
